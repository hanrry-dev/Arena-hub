...
const userProfileButton = document.querySelector('.sidebar-footer .user-profile');
const userNameLabel = document.querySelector('.sidebar-footer .user-info .name');
const userStatusLabel = document.querySelector('.sidebar-footer .user-info .status');
const userAvatar = document.querySelector('.sidebar-footer .avatar');
const loginIcon = document.querySelector('.sidebar-footer .login-icon');
...
function applyLoginState(userName) {
    ...
    userNameLabel.innerText = displayName;
    userStatusLabel.innerText = `Logado como ${displayName}`;
    loginIcon.classList.remove('ph-sign-in');
    loginIcon.classList.add('ph-sign-out');
    userAvatar.innerHTML = `<span>${displayName[0]?.toUpperCase() || 'A'}</span>`;
    localStorage.setItem('arenaHubUserName', displayName);
}

function loadLoginState() {
    const savedName = localStorage.getItem('arenaHubUserName');
    if (savedName) {
        applyLoginState(savedName);
    }
}
...
function openLoginModal() {
    ...
    setTimeout(() => loginNameInput.focus(), 80);
}
...
function setupEventListeners() {
    ...
    if (userProfileButton) {
        userProfileButton.addEventListener('click', openLoginModal);
    }
    ...
    if (loginConfirmBtn) {
        loginConfirmBtn.addEventListener('click', () => {
            const value = loginNameInput.value.trim();
            if (!value) {
                loginNameInput.focus();
                return;
            }
            applyLoginState(value);
            closeLoginModal();
        });
    }
